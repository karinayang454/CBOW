    ...
    for root, dirs, fns in os.walk(d):
        for fn in fns:
            if fn.split(".")[-1] == "txt":
                ...
                with open(os.path.join(d, fn), "r", encoding="utf8") as f:
...
def create_input_outputs(encoded_sentences, lens, k):
    # input = prior 2 and after 2 words, output = word
    size = 0
    for l in lens:
        if l >= (2*k+1):
            size += l - (2*k)
    input = np.zeros((int(size), int(k*2)), dtype=np.int32)
    output = np.zeros((int(size), 1), dtype=np.int32)
    ind = 0
    for i,es in enumerate(encoded_sentences):
        if (len(es) >= (2*k+1)):
            for j in range(2, lens[i][0] - 2):
                input[ind] = [es[j - 2], es[j - 1],
                        es[j + 1], es[j + 2]]
                output[ind] = es[j]
                ind += 1
    return input, output
